...
| 卡牌 | 费用/类型/稀有度 | 效果 |
|------|------------------|------|
| 余烬鳞甲 | 1/技/不常见 | 6 格挡，每有一名带灼燃敌人额外 +2 格挡 |
| 灰烬之拥 | 2/能/稀有 | 回合开始获得 灰烬层数 × 层数 的格挡 |
| 灼燃引爆 | 2/攻/稀有 | 8 伤 + 目标每层灼燃 4 伤，然后清除其灼燃 |
| 逆火 | 1/攻/稀有 | 全体 4 伤 + 每层灼燃计数器 2 伤，然后清空计数器 |
| 烬火余韵 | 1/能/稀有 | 每消耗一张牌抽 1（每回合上限 3/升级 4） |
| 余温 | 1/技/普通 | 抽 2，有灼燃计数器则再抽 1 |
| 殉焰 | 2/攻/不常见 | 8 伤并施加余烬蔓延（死亡时灼燃转移） |
| 龙裔威压 | 2/能/稀有 | 回合开始对带灼燃敌人施加等量虚弱 |
| 煽焰 | 0/技/普通 | 3 层灼燃计数器 + 抽 1，消耗 |
| 龙枪贯日 | 2/攻/稀有 | 8 伤 + 每点力量 2 伤 |
| 焰衡 | 1/攻/不常见 | 6 伤，目标灼燃为偶数层则翻倍 |
| 烬火长燃 | 2/能/稀有 | 敌人灼燃不再随回合衰减（规则改写） |
| 巨龙重压 | 3/攻/不常见 | 18 伤 + 每层灼燃计数器 3 伤，然后清空计数器 |

### 新增能力（4 个）

- 灰烬之拥 / 烬火余韵 / 龙裔威压 / 烬火长燃（对应上表）

### 初始卡组

- 加入 1 张火星（0 费 3 伤 + 1 灼燃，消耗），开局 9 张，第一场战斗即可体验灼燃机制

### 平衡性加强

- **灰烬**：每回合格挡从每 2 层 1 点改为**每层 1 点**
- **灰烬核心**（初始遗物）：新增战斗开始时获得 3 层灰烬
- **苇草之锋**（初始遗物）：回合开始全体灼燃 2 → 3 层
- 初始生命 72 → 78

### 机制说明

- 灼燃衰减逻辑新增烬火长燃检查；原版力量查询改用 `creature.GetPowerAmount<T>()` 直调
